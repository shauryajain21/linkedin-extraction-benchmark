"""
Generate the benchmark charts from the same data eval.py uses.

Reuses load()/completeness()/match() from eval.py so the numbers in the
charts are guaranteed to match results/benchmark_500.xlsx.

Writes PNGs to charts/:
  completeness.png     — mean completeness score per provider
  correctness.png      — right-person / wrong-namesake / empty, stacked, per provider
  field_fill.png       — per-field fill rate, grouped by provider
...
LINKUP = "#1d6fe0"   # highlight
GREY   = "#9aa3ad"   # everyone else
RIGHT  = "#34a853"   # green
WRONG  = "#ea4335"   # red
EMPTY  = "#cfd4da"   # grey
...
def chart_completeness(comp):
    order = sorted(PROVS, key=lambda p: mean(comp[p]))   # ascending -> Linkup on top
...
def chart_correctness(corr):
    order = sorted(PROVS, key=lambda p: corr[p]["right"])   # ascending -> Linkup on top
