"""
LinkedIn profile-extraction benchmark — completeness + correctness eval.

Reads:
  data/api_outputs_500.csv   — each API's structured response per profile (provider,endpoint,query,response,params)
  data/ground_truth_500.csv  — authoritative profile fields per linkedin_url (the source of truth)

Writes:
  results/benchmark_500.xlsx — Summary + per-person Completeness + per-person Correctness
...
def jload(s):
    """Parse the API response cell (may be raw JSON or fenced)."""
    s = (s or "").strip()
    if "```" in s:
        for p in s.split("```"):
...
# fuzzy company/title/name matching (token-set, suffix-stripped)
...
def match(a, b):
    """True/False if comparable, None if either side empty."""
...
# ----------------------------------------------------------------------------- completeness rubric
# 10 fields the query asks for. Each filled field = 1 point. Score = filled/10 * 100.
